...
use mqtt_codec::{Packet, Codec, Packet::*, ConnectCode::*};
...
impl Client {
    ...
    async fn handle_messages(framed: &mut Framed<TcpStream, Codec>, tx_broker: &mut Sender<BrokerMessage>, client_key: String, packet: Packet) {
        match packet {
            PingRequest => {
                println!("Ping");
            ...
            _ => {
                tx_broker.send(Message{id: client_key, packet}).await;
            }
        }
    }

    pub async fn handle_client(stream: TcpStream, addr: SocketAddr, tx_broker: &mut Sender<BrokerMessage>) {
        ...
        let packet = match framed.next().await {
            Some(Ok(Connect(packet))) => {
                framed.send(ConnectAck{session_present: false, return_code: ConnectionAccepted}).await;
                packet
            },
            _ => {
